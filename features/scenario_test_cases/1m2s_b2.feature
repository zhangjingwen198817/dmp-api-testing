Feature: 1m2s_b2
  ...
  @case.1012 @version.09
  Scenario: 1012-add_group should succeed
    When I add a MySQL group
    Then the response is ok
    And the MySQL group list should contains the MySQL group

  @case.1018 @version.09
  Scenario: 1018-config the MySQL group with SIP
    When I found 1m2s-B2 MySQL group with 3 MySQL instances,or I skip the test
  ...
  @case.1036 @version.09
  Scenario: 1036-bind/unbind SLA protocol to the MySQL group
    When I found 1m2s-B2 MySQL group with 3 MySQL instances,or I skip the test
  ...
  @case.1136  @version.09
  Scenario: 1136-config the MySQL instance with SIP
    When I found 1m2s-B2 MySQL group with 3 MySQL instances,or I skip the test
