Feature: 1m2s_b4
  ...
  @case.1020 @version.09
  Scenario: 1020-config the MySQL group with SIP
    When I found 1m2s-B4 MySQL group with 3 MySQL instances,or I skip the test
  ...
  @case.1065 @version.09
  Scenario: 1065-config the MySQL instance with SIP
    When I found 1m2s-B4 MySQL group with 3 MySQL instances,or I skip the test
  ...
  @case.1140 @version.09
  Scenario: 1140-trigger diagnosis score report
    When I found 1m2s-B4 MySQL group with 3 MySQL instances,or I skip the test
    And I found a MySQL master instance in the group
    And I execute the MySQL instance "create database action_db;create table action_db.char_columns(id int(11) not null primary key,c_char char(255)) ENGINE=MyISAM DEFAULT CHARSET=latin1;"
    When I trigger diagnosis score report
    Then the response is ok
    Then the diagnosis score list should contains the diagnosis score

  @case.1141 @version.09
  Scenario: 1141-trigger diagnosis report
    When I found 1m2s-B4 MySQL group with 3 MySQL instances,or I skip the test
    And I found a MySQL master instance in the group
